...
class NewFeedListTest(TestCase):
    def setUp(self):
        self.newsfeed =NewsFeed.objects.create(
            title ='Dogs',
            content='Dogs are animals that live in people house'
        )
        self.newsfeed2=NewsFeed.objects.create(
            title= 'Cats',
            content='Cats are animals that i so much love and cherish'
        )
        self.url=reverse('NewsFeed_create')
        
        

    def test_get_newfeed_list(self):
        response=self.client.get(reverse('NewsFeed_create'))
